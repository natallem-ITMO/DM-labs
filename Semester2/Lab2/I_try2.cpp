...
int main() {
    std::ifstream cin("fastminimization.in");
    std::ofstream cout("fastminimization.out");
    ...
    itr_set_int = terminal_states.begin();
    itr_set_int  = itr_set_int + 1;
    for (int i = 0; i <= n; i++) {
    ...
    if (terminal_states.empty() || !normal_vertex[1]) {
        ...
        for (itr_set_int = terminal_states.begin(); itr_set_int != terminal_states.end(); itr_set_int++)
            terminal_output.insert(class_by_vertex[*itr_set_int] + 1);
        cout << equivalence_classes.size() << " " << edges_output.size() << " " << terminal_output.size() << "\n";
        for (int x: terminal_output) {
            cout << x << " ";
        }
        cout << "\n";
